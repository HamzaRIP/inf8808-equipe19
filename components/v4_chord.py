...
GAP     = 0.03   # espace (radians) entre les arcs de genre
R_OUT   = 1.0    # rayon extérieur des arcs
R_IN    = 0.88   # rayon intérieur (départ des rubans)
R_LABEL = 1.13   # rayon des étiquettes
...
def bezier_ribbon(x0, y0, x1, y1, n=80):
    """Bézier cubique vers l'origine, crée le ruban intérieur"""
    t = np.linspace(0, 1, n)
    cx, cy = 0.0, 0.0
...
def compute_layout(genres, genre_totals, co_matrix):
    ...
    for g in genres:
        starts[g] = cursor
        ends[g]   = cursor + arc_sizes[g]
        cursor    = ends[g] + GAP

    ribbon_angles  = {}
    genre_cursors  = {g: starts[g] for g in genres}
    ...
    for i, g1 in enumerate(genres):
        for j, g2 in enumerate(genres):
            ...
            a1s   = genre_cursors[g1]
            a1e   = a1s + span1
            genre_cursors[g1] = a1e

            span2 = (val / genre_totals[g2]) * arc_sizes[g2]
            a2s   = genre_cursors[g2]
            a2e   = a2s + span2
            genre_cursors[g2] = a2e
...
def render(df):
    ...
    co_matrix    = [
        [int(co_full[i][j]) if i != j else 0 for j in range(len(genres))]
    ...
    for (g1, g2), (a1s, a1e, a2s, a2e) in ribbon_angles.items():
        ...
        pct1 = val / genre_totals[g1] * 100
        pct2 = val / genre_totals[g2] * 100
        color = GENRE_COLORS.get(g1, '#888888')

        ax1, ay1 = arc_points(a1s, a1e, R_IN)
        ax2, ay2 = arc_points(a2e, a2s, R_IN)
        ...
        hover = (
            f"<b>{g1.upper()} ↔ {g2.upper()}</b><br>"
            f"Titres partagés : <b>{val:,}</b><br>"
            f"Part de {g1} : {pct1:.1f} %<br>"
            f"Part de {g2} : {pct2:.1f} %"
        )
        ...
        traces.append(go.Scatter(
            ...
            line=dict(color=rgba(color, 0.5), width=0.8),
            hoverinfo='text',
            hovertext=hover,
    ...
    for g in genres:
        ...
        px = np.concatenate([ax, axi, [ax[0]]])
        py = np.concatenate([ay, ayi, [ay[0]]])

        pct   = genre_totals[g] / total_tracks * 100
        hover = (
            f"<b>{g.upper()}</b><br>"
            f"Titres : <b>{genre_totals[g]:,}</b><br>"
    ...
    for g in genres:
        ...
        annotations.append(dict(
            ...
            text=(
                f"<b>{g.upper()}</b><br>"
                f"<span style='font-size:9px;color:{MUTED}'>"
                f"{genre_totals[g]:,} titres</span>"
            ),
            showarrow=False,
